//#region node_modules/.nitro/vite/services/ssr/assets/demo-data-DrFUwAPu.js
var DEMO_PROJECT_ID = "cosy-demo-landing";
var DEMO_PROJECTS = [
	{
		id: DEMO_PROJECT_ID,
		title: "Warm Landing",
	...
	},
	{
		...
		updatedAt: (/* @__PURE__ */ new Date(Date.now() - 1e3 * 60 * 60 * 5)).toISOString(),
		isPublic: true,
		fileCount: 6,
	...
	},
	{
		...
		updatedAt: (/* @__PURE__ */ new Date(Date.now() - 1e3 * 60 * 60 * 26)).toISOString(),
		isPublic: false,
		fileCount: 8,
...
var DEMO_PROPOSAL_MODIFIED = `export function App() {
  return (
    <main className="min-h-screen bg-cream text-charcoal p-8">
...
`;
function buildPreviewHtml(files, deviceNote) {
	const html = files.get("index.html") ?? INDEX_HTML;
	const css = files.get("styles.css") ?? STYLES_CSS;
	return html.replace(/<link rel="stylesheet" href="\.\/styles\.css" \/>/, `<style>${css}</style>`).replace("</body>", `${deviceNote ? `<!-- ${deviceNote} -->` : ""}</body>`);
}
//#endregion
export { buildPreviewHtml as a, DEMO_PROPOSAL_MODIFIED as i, DEMO_PROJECTS as n, DEMO_PROJECT_ID as r, DEMO_FILES as t };
