...
function ve(e) {
	...
	return n === 0 ? t.map((o) => ({
		...
		let u = 0;
		if (s.collapsedSize !== void 0) u = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.collapsedSize
		}) / n * 100);
		let a;
		if (s.defaultSize !== void 0) a = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.defaultSize
		}) / n * 100);
		let r = 0;
		if (s.minSize !== void 0) r = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.minSize
		}) / n * 100);
		let l = 100;
		if (s.maxSize !== void 0) l = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.maxSize
		}) / n * 100);
...
function Ze(e) {
	const { element: t, orientation: n, panels: o, separators: i } = e, s = be(n, Array.from(t.children).filter(Ye).map((z) => ({ element: z }))).map(({ element: z }) => z), u = [];
...
function le({ delta: e, initialLayout: t, panelConstraints: n, pivotIndices: o, prevLayout: i, trigger: s }) {
		...
		case "keyboard":
			{
				...
				if (y) {
					const v = a[f];
					if (C(v != null, `Previous layout not found for panel index ${f}`), k(v, h)) {
						const g = b - v;
			...
			}
			{
				...
				if (y) {
					const v = a[f];
					if (C(v != null, `Previous layout not found for panel index ${f}`), k(v, b)) {
						const g = v - h;
						A(g, Math.abs(e)) > 0 && (e = e < 0 ? 0 - g : g);
					}
				}
	...
	}
	{
		...
		for (; d >= 0 && d < n.length;) {
			...
			if (!k(y, v) && (p += y - v, l[d] = v, p.toFixed(3).localeCompare(Math.abs(e).toFixed(3), void 0, { numeric: !0 }) >= 0)) break;
			e < 0 ? d-- : d++;
		}
	}
	if ($t(r, l)) return i;
	{
		...
		if (l[f] = y, !k(y, h)) {
			...
			for (; g >= 0 && g < n.length;) {
				...
				if (k(w, L) || (b -= L - w, l[g] = L), k(b, 0)) break;
				e > 0 ? g-- : g++;
			}
		}
	}
	if (!k(Object.values(l).reduce((f, d) => d + f, 0), 100, .1)) return i;
...
function K({ layout: e, panelConstraints: t }) {
	...
	if (!k(i, 100) && o.length > 0) for (let a = 0; a < t.length; a++) {
		const r = o[a];
	...
	if (!k(s, 0)) for (let a = 0; a < t.length; a++) {
		const r = o[a];
		C(r != null, `No layout data found for index ${a}`);
		const l = r + s, c = Z({
			overrideDisabledPanels: !0,
		...
		if (r !== c && (s -= c - r, o[a] = c, k(s, 0))) break;
	}
...
function at({ groupId: e, panelId: t }) {
	const n = () => {
		...
		if (r === s()) return;
		const { defaultLayoutDeferred: c, derivedPanelConstraints: m, group: p, groupSize: S, layout: z, separatorToPanels: f } = n(), h = K({
			layout: u({
				nextSize: r,
				panels: p.panels,
				prevLayout: z,
	...
	return {
		...
		resize: (r) => {
			const { group: l } = n(), { element: c } = i(), m = ne({ group: l }), S = T(ie({
				groupSize: m,
				panelElement: c,
				styleProp: r
			}) / m * 100);
...
function lt({ groupId: e }) {
	...
	return {
		...
		setLayout(n) {
			const { defaultLayoutDeferred: o, derivedPanelConstraints: i, group: s, groupSize: u, layout: a, separatorToPanels: r } = t(), l = K({
				layout: n,
				panelConstraints: i
			});
...
function U(e, t) {
	const n = pe(e), o = H(n.id, !0), i = n.separators.find((m) => m.element === e);
	C(i, "Matching separator not found");
	const s = o.separatorToPanels.get(i);
	C(s, "Matching panels not found");
	const u = s.map((m) => n.panels.indexOf(m)), r = lt({ groupId: n.id }).getLayout(), c = K({
		layout: le({
			delta: t,
			initialLayout: r,
...
function $e(e) {
	...
	if (!n.disabled) switch (e.key) {
		...
		case "Enter": {
			e.preventDefault();
			const o = pe(t), { derivedPanelConstraints: s, layout: u, separatorToPanels: a } = H(o.id, !0), r = o.separators.find((p) => p.element === t);
			...
			if (C(m, "Panel metadata not found"), m.collapsible) {
				const p = u[c.id];
				U(t, (m.collapsedSize === p ? o.mutableState.expandedPanelSizes[c.id] ?? m.minSize : m.collapsedSize) - p);
			}
			break;
		}
		case "F6": {
		...
		for (const d of f) {
			...
			if (y === e.element) {
				if (t) {
					...
					const v = H(e.id);
					if (!v) return;
					const g = ve(e), w = v.defaultLayoutDeferred ? We(g) : v.layout, L = K({
						layout: Vt({
							group: e,
							nextGroupSize: b,
					...
					if (!v.defaultLayoutDeferred && W(v.layout, L) && Ht(v.derivedPanelConstraints, g) && v.groupSize === b) return;
					j(e, {
...
function Xt({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: u, id: a, onLayoutChange: r, onLayoutChanged: l, orientation: c = "horizontal", resizeTargetMinimumSize: m = {
	coarse: 20,
	fine: 10
}, style: p, ...S }) {
	...
	}), h = Le(a), y = (0, import_react.useRef)(null), [b, v] = Wt(), g = (0, import_react.useRef)({
	...
	const M = se((x, P) => {
		...
		if (E) {
			...
			switch (I.state) {
				case "active":
					D = I.hitRegions.some((V) => V.group === R);
					break;
			}
			return {
		...
		registerPanel: (x) => {
			const P = g.current;
			return P.panels = be(c, [...P.panels, x]), v(), () => {
				P.panels = P.panels.filter((I) => I !== x), v();
			};
		},
		registerSeparator: (x) => {
			const P = g.current;
			return P.separators = be(c, [...P.separators, x]), v(), () => {
		...
		const oe = Pe(h, (_) => {
			...
			R.mutableState.layouts[ft] = ce, Ee.forEach(($) => {
				if ($.collapsible) {
					...
					if (ge) {
						const pt = k($.collapsedSize, ce[$.panelId]), ht = k($.collapsedSize, ge[$.panelId]);
...
function Zt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: u, groupResizeBehavior: a = "preserve-relative-size", id: r, maxSize: l = "100%", minSize: c = "0%", onResize: m, panelRef: p, style: S, ...z }) {
	const f = !!r, d = Le(r), h = Re({ disabled: s }), y = (0, import_react.useRef)(null), b = Ce(y, u), { getPanelStyles: v, id: g, orientation: w, registerPanel: M, updatePanelProps: L } = Me(), G = m !== null, N = se((R, E, D) => {
		m?.(R, r, D);
	});
	q(() => {
		const R = y.current;
	...
	}, P = (0, import_react.useSyncExternalStore)((R) => Pe(g, R), x, x);
	let I;
...
function en({ layout: e, panelConstraints: t, panelId: n, panelIndex: o }) {
	let i, s;
...
function tn({ children: e, className: t, disabled: n, disableDoubleClick: o, elementRef: i, id: s, style: u, ...a }) {
	const r = Le(s), l = Re({
		disabled: n,
		disableDoubleClick: o
	}), [c, m] = (0, import_react.useState)({}), [p, S] = (0, import_react.useState)("inactive"), [z, f] = (0, import_react.useState)(!1), d = (0, import_react.useRef)(null), h = Ce(d, i), { disableCursor: y, id: b, orientation: v, registerSeparator: g, updateSeparatorProps: w } = Me(), M = v === "horizontal" ? "vertical" : "horizontal";
	q(() => {
		const N = d.current;
		if (N !== null) {
			const x = {
...
//#endregion
export { Zt as n, tn as r, Xt as t };
